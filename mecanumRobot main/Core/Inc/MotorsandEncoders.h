...
void PreciseWheelPositionInit() {
	...
	WheelPIDControllersInit(20.0f, -0.5f, 0.008f,  // Front Left (Kp, Ki, Kd)
			20.0f, -0.5f, 0.008f,  // Front Right
			20.0f, -0.5f, 0.008f,  // Rear Left
			20.0f, -0.5f, 0.008f   // Rear Right
			);

	// Method 2: PID gains for robot pose control (x, y, theta)
	// These values need tuning for your specific robot
	PosePIDControllersInit(1.0f, 0.1f, 0.2f,    // X position (was 0.8f for Kp)
			1.0f, 0.1f, 0.2f,    // Y position (was 0.8f for Kp)
			2.0f, 0.1f, 1.0f     // Theta (was 2.0f for Kp)
			);
...
void UpdateEncoderCounts(void) {
	...
	// Update total counts
	flTotalCount += flDelta;
	frTotalCount += frDelta;
	rlTotalCount += rlDelta;
	rrTotalCount += rrDelta;
...
//	UpdateSensorData();
	...
	currentX += vx_global * deltaTimeSec;
...
void SetMotorSpeed(uint8_t motor, int16_t speed) {
	...
	case 0: // Front Left
		...
		break;
	case 1: // Front Right
		in1Port = FR_IN1_GPIO_Port;
		in1Pin = FR_IN1_Pin;
		in2Port = FR_IN2_GPIO_Port;
		in2Pin = FR_IN2_Pin;
...
void TestMotorDirections() {
	...
	SetMotorSpeed(0, 500);  // Front Left
	HAL_Delay(1000);
	SetMotorSpeed(1, 500);  // Front Right
	HAL_Delay(1000);
	SetMotorSpeed(2, 500);  // Rear Left
	HAL_Delay(1000);
	SetMotorSpeed(3, 500);  // Rear Right
